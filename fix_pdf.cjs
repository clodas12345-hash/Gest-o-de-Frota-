...
const newUrlToDataUrl = `async function urlToDataUrl(url: string): Promise<{ dataUrl: string; format: 'PNG' | 'JPEG' } | null> {
  if (!url) return null;
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        // Scale down if too large to prevent huge PDFs
    ...
    img.onerror = () => {
      // Fallback on error if it's already a base64 string
      if (url.startsWith('data:image/png')) {
        resolve({ dataUrl: url, format: 'PNG' });
      } else if (url.startsWith('data:image/jpeg') || url.startsWith('data:image/jpg')) {
        resolve({ dataUrl: url, format: 'JPEG' });
...
content = content.replace(/async function urlToDataUrl[\s\S]*?if \(url\.startsWith\('http'\)[\s\S]*?\}\s*\}/, newUrlToDataUrl);
